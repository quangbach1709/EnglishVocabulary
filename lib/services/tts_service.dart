...
class TtsService {
  ...
  Future<void> init() async {
    // Hive is not used on web; settings are kept in memory only.
    if (!kIsWeb) {
  ...
  Future<void> setSpeechRate(double rate) async {
    ...
    if (!kIsWeb) await _settingsBox.put('tts_speechRate', _speechRate);
  }

  /// Update language/accent and persist
  Future<void> setLanguage(String lang) async {
    _language = lang;
    if (!kIsWeb) await _settingsBox.put('tts_language', _language);
  }
}
